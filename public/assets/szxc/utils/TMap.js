...
class TMap {
  ...
  constructor(el, obj) {
    ...
    const imageURLT = "http://t0.tianditu.gov.cn/cia_w/wmts?" +
      ...
      "&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}" +
      "&tk=ae735d13cbec178839eb32925ca76e7c";
    const lay = new T.TileLayer(imageURL, { minZoom: 7, maxZoom: 18 });
    const lay2 = new T.TileLayer(imageURLT, { minZoom: 7, maxZoom: 18 });
  ...
  // 可视区域划分
  areaShow(point,point1) {
    var points = [];
    points = point.map(item => new T.LngLat(item[0], item[1]))

    var points1 = [];
    points1 = point1.map(item => new T.LngLat(item[0],item[1]))
    //创建面对象
    var polygon = new T.Polygon([points,points1],{
      color: "#fff", weight: 1, opacity: 1, fillColor: "#010d31", fillOpacity:1
    });
    //向地图上添加面
    map.addOverLay(polygon);
  }
